...
public class Geocalc
{
	 ...
	 * @return
	 */
	public static double distanceRayToLineSegment2D(BaseLineSegment line, Vec3D origin, Vec3D direction)
	{
		Vec3D tangent = line.computeTangent();
		Vec3D projection = projectPointOntoRay(origin, line.p1, tangent);
		...
		Vec3D norm = line.computeNorm();
		return Math.abs(Vec3D.distance(projection, origin) / Vec3D.dot(norm, direction));
	}
}
